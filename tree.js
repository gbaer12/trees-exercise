...
class Tree {
  ...
  numGreater(lowerBound) {
    ...
    function numGreatersHelper(node) {
      //go through all the children for a Node
      for (let child of node.children) {
        //count the vhild if the value is greater than lowerBound
        if (child.val > lowerBound) count++;
        //if it has any children
        if (child.children.length > 0) {
          //recurse with the child as the root
          numGreatersHelper(child);
        }
      }
    }
    numGreatersHelper(this.root);
    return count;
  }
}
